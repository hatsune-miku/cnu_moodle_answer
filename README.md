...
每份解答都由纯 C 语言编写，都可以通过所有的测试用例(Accepted).
~辣鸡平台不给C++用，没有STL我要死了~
所有的解答都是个人编写的！所以，只能作为参考，不要让我的程序限制你的思路。
希望大家不要盲目照搬，毕竟参考解答**不一定是最优解**. ~甚至还有些写的很烂~<br/>

源程序保证没有语法错误。如果它在你的开发环境中无法通过编译，说明你的编译器过时了。
